...
namespace labutils
{
	...
	VulkanWindow make_vulkan_window()
	{
		...
		for (std::uint32_t i = 0; i < reqExtCount; ++i)
		{
			...
			{
				throw lut::Error("GLFW/Vulkan: required instance extension %s not "
					"supported", requiredExt[i]);
			}
...
#		endif // ~ debug builds
		...
		ret.window = glfwCreateWindow(1280, 720, "CourseWork3_YuSi", nullptr, nullptr);
		if (!ret.window)
		{
			char const* errMsg = nullptr;
			glfwGetError(&errMsg);
		...
		if (auto const res = glfwCreateWindowSurface(ret.instance, ret.window,
			nullptr, &ret.surface); VK_SUCCESS != res) {
	...
	SwapChanges recreate_swapchain(VulkanWindow& aWindow)
	{
		...
		std::vector<std::uint32_t> queueFamiliesIndices;
		if (aWindow.presentFamilyIndex != aWindow.graphicsFamilyIndex)
		{
			queueFamiliesIndices.emplace_back(aWindow.graphicsFamilyIndex);
			queueFamiliesIndices.emplace_back(aWindow.presentFamilyIndex);
		}
		...
		try
		{
			std::tie(aWindow.swapchain, aWindow.swapchainFormat, aWindow.swapchainExtent)
				=
				create_swapchain(aWindow.physicalDevice, aWindow.surface, aWindow.device, aWindow.window, queueFamiliesIndices, oldSwapchain);
		}
		catch (...)
		{
			// Put pack the old swap chain handle into the Vulkan Window
			// this ensures that the old swap chain is destroyed when this error branch
			// occures
			aWindow.swapchain = oldSwapchain;
			throw;
		}
		//destory the old swap chain
		vkDestroySwapchainKHR(aWindow.device, oldSwapchain, nullptr);

		//get new swap chain image and create associated image view
		...
		// Determine which swap chain properties have changed and return the information
		// indicating this
		SwapChanges ret{};
...
namespace
{
	std::vector<VkSurfaceFormatKHR> get_surface_formats(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &numSurfaceFormat, nullptr); VK_SUCCESS != res)
		{
		...
		if (auto const res = vkGetPhysicalDeviceSurfaceFormatsKHR(aPhysicalDev, aSurface, &numSurfaceFormat, surfaceFormats.data()); VK_SUCCESS != res)
		{
	...
	std::unordered_set<VkPresentModeKHR> get_present_modes(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface)
	{
		...
		if (auto const res = vkGetPhysicalDeviceSurfacePresentModesKHR(aPhysicalDev, aSurface, &numPresentModes, nullptr); VK_SUCCESS != res)
		{
		...
		if (auto const res = vkGetPhysicalDeviceSurfacePresentModesKHR(aPhysicalDev, aSurface, &numPresentModes, presentModes.data()); VK_SUCCESS != res)
		{
	...
	std::tuple<VkSwapchainKHR, VkFormat, VkExtent2D> create_swapchain(VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain)
	{
		...
		VkSurfaceFormatKHR format = formats[0]; // FIXME!
		...
		if (std::numeric_limits<std::uint32_t>::max() == extent.width)
		{
			...
			//ensure the extent is within the range defined
	...
	void get_swapchain_images(VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages)
	{
		...
		{
			throw lut::Error("Error : Unable to get swapchain images\n""vkGetSwapchainImagesKHR() returned %s", lut::to_string(res).c_str());
		}
	...
	void create_swapchain_image_views(VkDevice aDevice, VkFormat aSwapchainFormat, std::vector<VkImage> const& aImages, std::vector<VkImageView>& aViews)
	{
		...
		for (size_t i = 0; i < aImages.size(); i++)
		{
			...
			{
				throw lut::Error("Error : Unable to create image view for swap chain image %zu\n""vkCreateImageView() returned %s", i, lut::to_string(res).c_str());
			}
