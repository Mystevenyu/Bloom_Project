...
void main() {
    ...
    for (int i = 1; i <= GAUSSIAN_SAMPLES; i+=2) 
    {
        ...
        vec3 sample1 = textureClampToEdge(uTexColor, inUV + vec2(0.0,tex_offset.y * i)).rgb;
        vec3 sample2 = textureClampToEdge(uTexColor, inUV + vec2(0.0,tex_offset.y * (i + 1))).rgb;
        vec3 combinedSample = mix(sample1, sample2, weight2 / combinedWeight);

        result += combinedSample * combinedWeight;

        sample1 = textureClampToEdge(uTexColor, inUV - vec2(0.0,tex_offset.y * i)).rgb;
        sample2 = textureClampToEdge(uTexColor, inUV - vec2(0.0,tex_offset.y * (i + 1))).rgb;
