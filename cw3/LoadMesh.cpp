...
LoadModel create_loaded_mesh(labutils::VulkanContext const& aContext, labutils::Allocator const& aAllocator, VkDescriptorPool& dpool, VkCommandPool& aCmdPool, VkDescriptorSetLayout& aObjectLayout, VkSampler& aSampler, BakedModel const& shipModel)
{
	//final return
	LoadModel loadedModel;
	
	for (auto& mesh : shipModel.meshes)
	{
	...
	for (size_t i = 0; i < shipModel.textures.size(); i++)
	{
		lut::Image image = lut::load_image_texture2d(shipModel.textures[i].path.c_str(), aContext, aCmdPool, aAllocator);
	...
	uint32_t matCount = static_cast<uint32_t>(shipModel.materials.size());
	...
	for (uint32_t i = 0; i < matCount; i++)
	{
		...
		imageInfo[0].imageView = loadedModel.imageViews[shipModel.materials[i].baseColorTextureId].handle;
		imageInfo[0].sampler = aSampler;

		imageInfo[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[1].imageView = loadedModel.imageViews[shipModel.materials[i].roughnessTextureId].handle;
		imageInfo[1].sampler = aSampler;

		imageInfo[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		imageInfo[2].imageView = loadedModel.imageViews[shipModel.materials[i].metalnessTextureId].handle;
	...
	std::vector<NewMaterialInfo> newMaterialInfos(shipModel.materials.size());
	for (uint32_t i = 0; i < newMaterialInfos.size(); ++i)
	{	
		const auto& bakedMaterial = shipModel.materials[i];
		loadedModel.newMaterialInfo.emplace_back(std::move(from_baked_material(bakedMaterial)));
	}
...
NewMaterialInfo from_baked_material(const BakedMaterialInfo& bakedMaterial)
{
	...
	newMaterialInfo.baseColor = bakedMaterial.baseColor;
	newMaterialInfo.roughness = bakedMaterial.roughness;
	newMaterialInfo.emissiveColor = bakedMaterial.emissiveColor;
	newMaterialInfo.metalness = bakedMaterial.metalness;
